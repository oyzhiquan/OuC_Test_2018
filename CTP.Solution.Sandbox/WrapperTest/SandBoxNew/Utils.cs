...
namespace WrapperTest
{
    ...
    public static class Utils
    {
                ...
                if (InstrumentToQuotes.ContainsKey(instrumentId))
                {
                    ...
                    if (preQuotes.Count > 0)
                    {
                        ...
                        if (minuteByMinuteChart.Count > 0) //最后一个行情是最新一分钟的
                        {
                            ...
                            {
                                //最新价大于上次最新价，则上涨次数++
                                if (tuple.Item2.AveragePrice > lastMinuteQuote.Item2.AveragePrice)
                                {
                                    if (InstrumentUpDownCount.ContainsKey(instrumentId))
                                    {
                                        InstrumentUpDownCount[instrumentId].UpCount++;
                                        InstrumentUpDownCount[instrumentId].DownCount = 0;
                                    }
                                    else
                                    {
                                        InstrumentUpDownCount.TryAdd(instrumentId, new UpDownCount());
                                        InstrumentUpDownCount[instrumentId].UpCount++;
                                        InstrumentUpDownCount[instrumentId].DownCount = 0;
                                    }
                                }
                                ...
                                if (tuple.Item2.AveragePrice < lastMinuteQuote.Item2.AveragePrice)
                                {
                                    if (InstrumentUpDownCount.ContainsKey(instrumentId))
                                    {
                                        InstrumentUpDownCount[instrumentId].DownCount++;
                                        InstrumentUpDownCount[instrumentId].UpCount = 0;
                                    }
                                    else
                                    {
                                        InstrumentUpDownCount.TryAdd(instrumentId, new UpDownCount());
        ...
        /// 合约最新价相比上次上涨和下跌次数
        /// </summary>
        public static ConcurrentDictionary<string, UpDownCount> InstrumentUpDownCount = new ConcurrentDictionary<string, UpDownCount>();
    }
